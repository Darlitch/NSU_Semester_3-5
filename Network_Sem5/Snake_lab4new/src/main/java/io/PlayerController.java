...
public class PlayerController {
    ...
    @Getter
    private final Subject<GameView.Control> controlSubject = PublishSubject.create();
    @Getter
            ...
            if (role == SnakesProto.NodeRole.MASTER) {
                if (msg.getMessage().hasSteer()) {
                    playersManager.getIdBySignature(signature).ifPresent(id -> controlSubject.onNext(
                            new GameView.Control(id, msg.getMessage().getSteer().getDirection())
                    ));
                }
    ...
    public void sendGameStateWorker() {
        while (true) {
            if (role == SnakesProto.NodeRole.MASTER) {
                ...
                state = gameController.getNextState(oldState);
